...
namespace Unity.DebugDisplay
{
    ...
    internal struct Arrows : IDisposable
    {
        ...
        internal void Draw(float3 x, float3 v, Unity.DebugDisplay.ColorIndex color)
        {
            ...
            float3 dir;
            float length = Scellecs.Morpeh.Physics.Math.NormalizeWithLength(v, out dir);
            float3 perp, perp2;
            Scellecs.Morpeh.Physics.Math.CalculatePerpendicularNormalized(dir, out perp, out perp2);
    ...
    internal struct Cones : IDisposable
    {
        ...
        {
            float3 dir;
            float scale = Scellecs.Morpeh.Physics.Math.NormalizeWithLength(axis, out dir);
            float3 arm;
            {
                float3 perp1, perp2;
                Scellecs.Morpeh.Physics.Math.CalculatePerpendicularNormalized(dir, out perp1, out perp2);
